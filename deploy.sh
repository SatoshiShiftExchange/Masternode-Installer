...
function set_environment()
{
  ...
  CONF_FILE="$DATADIR/satoshishift.conf"
  DAEMON_START="$DAEMON"
...
function check_existing()
{
  ...
  #Get number of existing satoshishift masternode directories
  DIR_COUNT=$(ls -la /root/ | grep "\.satoshishift" | grep -c '^')
...
echo -e " ${RED}■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■${NC}"
echo -e " ${YELLOW}$PROJECT Masternode Installer v$VERSION - Satoshishift 2019"
echo -e " ${RED}■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■${NC}"
echo
...
function copy_binaries()
{
    ...
    echo
    wget https://github.com/SatoshiShiftExchange/Satoshishift-Wallet/releases/download/v1.0.0.0/satoshishiftd > /dev/null 2>&1
    wget https://github.com/SatoshiShiftExchange/Satoshishift-Wallet/releases/download/v1.0.0.0/satoshishift-cli > /dev/null 2>&1
    chmod +x satoshishift-cli
...
function start_wallet()
{
  ...
  if [ -f $DAEMON ]; then
    ...
    echo -e "${BLUE} Next, go to your data folder and double-click on ${GREEN}SatoshiShift${BLUE} (see methods below)${NC}"
    echo
    echo -e " In Windows: Click the start button and type %APPDATA% and press ENTER, then double-click on the SatoshiShift folder"
    echo -e " In MacOS: with your file explorer, navigate to /Users/USERNAME/Library/Application Support/SatoshiShift"
    echo
    ...
    echo
    echo
    echo -e "$MN_ALIAS $NEXT_AVAIL_IP:20000 $GENKEY $TX_ID $TX_OUTPUT"
    echo
    echo
